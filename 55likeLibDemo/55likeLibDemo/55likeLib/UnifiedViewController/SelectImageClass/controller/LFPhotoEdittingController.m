...
{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(refreshEditDrawEndedBarState) object:nil];
...
- (void)lf_photoEditDrawEnded
{
    ...
    isEditDrawEnded=NO;
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(refreshEditDrawEndedBarState) object:nil];
    [self performSelector:@selector(refreshEditDrawEndedBarState) withObject:nil afterDelay:1.5];
...
-(void)refreshEditDrawEndedBarState{
    if (!isEditDrawEnded) {
        self.isHideNaviBar = NO;
        [self changedBarState];
    }
...
- (void)changedBarState
{
    ...
    [UIView animateWithDuration:.25f animations:^{
        CGFloat alpha = self.isHideNaviBar ? 0.f : 1.f;
