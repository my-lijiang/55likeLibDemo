...
- (void)initSubviews {
    ...
    if (_models.count == 0) {
        ...
        if (!imagePickerVc.showRadioImage) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.showSelectBtn = !imagePickerVc.showRadioImage;
    
    __weak typeof(self) weakSelf = self;
    __weak LFAssetCell *weakCell = cell;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (imagePickerVc.showRadioImage) {
        //单选图片裁剪
        WWlikeRadioImageCropViewController *photoEdittingVC=[[WWlikeRadioImageCropViewController alloc] init];
        
        /** 获取缓存编辑对象 */
        LFPhotoEdit *photoEdit = [[LFPhotoEditManager manager] photoEditForAsset:model];
        if (photoEdit) {
            photoEdittingVC.image = photoEdit.editImage;
        } else if (model.previewImage) { /** 读取自定义图片 */
            photoEdittingVC.image = model.previewImage;
        } else {
            /** 获取对应的图片 */
            [[LFAssetManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                photoEdittingVC.image = photo;
            }];
        }
